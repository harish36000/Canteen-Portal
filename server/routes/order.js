...
router.post("/rate", async (req, res) => {
    ...
    console.log("REQUEST TO POST RATE")
    console.log(req)
...
router.get("/stats", async (req, res) => {
    ...
    //vendor Products = [idli, vada, dosa]

    const user = await User.findOne({email: req.user.email}).populate('orders')
    if (!user) return res.json({status: 1, error: "User not found"})

    //user Orders = [order1: 1 idli, order2: 2 vada, order3: 3 idli]

    //Adding number of times a product is ordered 
    let pCnt = []
...
router.get("/batchstats", async (req, res) => {
    ...
    var test = {}
    var users = await Buyer.find({}) //returns all buyers
