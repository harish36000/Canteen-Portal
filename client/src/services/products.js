...
const AxiosGetProducts = async () => {
    ...
    if (!res) return null
    const productList = await axios.get("http://localhost:4000/api/product/")
...
const AxiosUpdateFavourite = async (id) => {
    const res = await axios.post("http://localhost:4000/api/product/favourite", {pid: id})
    if (!res || res.data.status === 1) console.log(res.data.error)
    return res.data
}

const AxiosGetMyProducts = async () => {
    const res = await axios.get("http://localhost:4000/api/product/my")
    if (!res || res.data.status === 1) console.log(res.data.error)
    return res.data
}

const AxiosDeleteProduct = async (val) => {
    const res = await axios.post("http://localhost:4000/api/product/delete", val)
    if (!res || res.data.status === 1) console.log(res.data.error)
    return res.data
}

const AxiosUpdateProduct = async (val) => {
    const res = await axios.post("http://localhost:4000/api/product/update", val)
    if (!res || res.data.status === 1) console.log(res.data.error)
    return res.data
}
...
const AxiosAddProduct = async (val) => {
    console.log("val---"+val)
    const res = await axios.post("http://localhost:4000/api/product/add", val)
    if (!res || res.data.status === 1) console.log(res.data.error)
    return res.data
}
